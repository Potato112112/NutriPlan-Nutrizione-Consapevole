...
import { Plus, Pencil, Trash2, X, Printer, ChevronDown, ChevronUp, ShoppingCart, Copy, ArrowUpDown } from "lucide-react";
  ...
  function updateDayForWeekday(dayOfWeek: number, dayId: string) {
    setForm((f) => ({
      ...
      weekDays: f.weekDays.map((wd) =>
        wd.dayOfWeek === dayOfWeek ? { ...wd, dayId: dayId as Id<"days"> | "", customSlots: undefined } : wd
      ),
    }));
  }
      ...
      <div className="space-y-3">
        {sortedWeeks.length === 0 && (
          <div className="text-center py-12 text-gray-400 bg-white rounded-xl border">
            Nessuna settimana alimentare creata
          </div>
        )}
...
function IngredientPicker({ value, onChange, ingredients }: { value: string; onChange: (id: string) => void; ingredients: any[] }) {
  ...
  if (selected) {
    return (
      <div className="flex items-center gap-2 border rounded-lg px-3 py-1.5 bg-emerald-50 border-emerald-200">
...
function WeekDayCustomizeModal({ dayOfWeek, form, setForm, selectedDaysWithMeals, ingredients, onClose }: any) {
  ...
  if (slots.length === 0 && selectedDay) {
    const initial = wd?.customSlots && wd.customSlots.length > 0
      ? wd.customSlots
  ...
  function saveCustomizations() {
    setForm((prev: WeekForm) => ({
      ...prev,
      weekDays: prev.weekDays.map((x) => x.dayOfWeek === dayOfWeek ? { ...x, customSlots: slots } : x),
    }));
    onClose();
  }
  ...
  function clearCustomizations() {
    setForm((prev: WeekForm) => ({
      ...prev,
      weekDays: prev.weekDays.map((x) => x.dayOfWeek === dayOfWeek ? { ...x, customSlots: undefined } : x),
    }));
    onClose();
  }
        ...
        <div className="p-5 space-y-4">
          {slots.map((slot, slotIdx) => (
            <div key={`${slot.mealId}_${slotIdx}`} className="border rounded-lg p-3">
              <div className="text-sm font-semibold text-gray-700 mb-2">{MEAL_TYPE_ICONS[slot.mealType]} {slot.mealName}</div>
              <div className="space-y-1.5">
                {(slot.items ?? []).map((item: any, itemIdx: number) => (
                  <div key={itemIdx} className="flex gap-2 items-center">
                    <div className="flex-1">
                      <IngredientPicker value={item.ingredientId} onChange={(id) => updateItem(slotIdx, itemIdx, "ingredientId", id)} ingredients={ingredients} />
                    </div>
